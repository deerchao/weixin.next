            ...
            protected override void DeserializeFields(List<KeyValuePair<string, string>> values)
            {
                ...
                if (result_code == result_success)
                {
                    ...
                    if (coupon_count > 0)
                    {
                        ...
                        for (var i = 0; i < coupon_count.Value; i++)
                        {
                            coupons[i] = new CouponPayment
                            {
                                coupon_type = (CouponType)Enum.Parse(typeof(CouponType), GetValue(values, "coupon_type_$" + i.ToString("D"))),
                                coupon_id = GetValue(values, "coupon_id_$" + i.ToString("D")),
                                coupon_fee = GetIntValue(values, "coupon_fee_$" + i.ToString("D")) ?? 0,
                            };
                        }
                    }
                }
